...
from preprocessing.remove_html_tags import RemoveHTMLTags


@pytest.fixture
def sample_messy_html():
    """Realistic messy boards.ie HTML"""
...
def expected_clean_text():
    return ("seeing as the UK has sold about £500m in arms in the last ten years, "
            "I’d be surprised if it is billions, especially as the UK accounts for "
            "less than 1% of arms sales to Israel. here’s the full response to the "
            "parliamentary question")


def test_remove_html_tags_single_text(sample_messy_html, expected_clean_text):
    cleaner = RemoveHTMLTags()
    cleaner.fit(None)  # ← Required because it's abstract

    df = pd.DataFrame({"comment": [sample_messy_html]})
    result_df = cleaner.transform(df)
    cleaned_text = result_df["comment"].iloc[0]
...
def test_remove_html_tags_empty_and_none():
    cleaner = RemoveHTMLTags()
    cleaner.fit(None)  # ← Required

    df = pd.DataFrame({
        "comment": ["", None, "   ", "<p>Normal clean text here.</p>"]
    })

    result = cleaner.transform(df)

    assert result["comment"].iloc[0] == ""
    assert result["comment"].iloc[1] == ""
    assert result["comment"].iloc[2] == ""
    assert "Normal clean text here" in result["comment"].iloc[3]


def test_remove_html_tags_multiple_rows(sample_messy_html):
    cleaner = RemoveHTMLTags(columns=["text"])
    cleaner.fit(None)  # ← Required

    df = pd.DataFrame({
        "text": [
            "<p>This is a normal comment without embeds.</p>",
            sample_messy_html,
            "<div class='js-embed'>Quoted text</div><p>Real reply text here.</p>"
        ]
    })

    result = cleaner.transform(df)

    assert "normal comment without embeds" in result["text"].iloc[0].lower()
    assert "here’s the full response to the parliamentary question" in result["text"].iloc[1]
    assert "Real reply text here" in result["text"].iloc[2]
